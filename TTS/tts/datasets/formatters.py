...
def hui(root_path, meta_files=None, ignored_speakers=None):
    """Normalize the HUI meta data file to TTS format
    https://opendata.iisys.de/dataset/hui-audio-corpus-german/"""
    items = []
    if not meta_files:
        meta_files = glob(f"{root_path}/**/metadata.csv", recursive=True)
    else:
        if isinstance(meta_files, str):
            meta_files = [os.path.join(root_path, meta_files)]
